...
export function publishedRates(
  ...
  return [
    ...
    { endpoint: '/api/v1/search/comments', variant: 'facebook', upstreamUsd: 0.12, priceUsd: priceFor(PRICE_FLOORS.search, 0.12), note: 'per post; ~$0.40/1k comments' },
    { endpoint: '/api/v1/search/comments', variant: 'x', upstreamUsd: 0.045, priceUsd: priceFor(PRICE_FLOORS.search, 0.045), note: 'per post; ~$0.15/1k replies' },
    { endpoint: '/api/v1/search/comments', variant: 'reddit', upstreamUsd: 1.02, priceUsd: priceFor(PRICE_FLOORS.search, 1.02), note: 'per post; actor start + per comment' },
